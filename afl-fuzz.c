...
AFLNET_REGIONS_SELECTION:;
  ...
  if (state_aware_mode) {
    ...
    if (target_state_id == 0) { //没有M1，全是M2
      ...
      for(i = 0; i < queue_cur->region_count; i++) {
        u32 regionalStateCount = queue_cur->regions[i].state_count; // state_count记录的是全部的状态码数量（包括无序的那些）
      ...
      for(i = M2_start_region_ID; i < queue_cur->region_count ; i++) {
        if (queue_cur->regions[i].state_count != queue_cur->regions[M2_start_region_ID].state_count) break; //同一个M2中的所有region的state_count是一样的
        M2_region_count++;
      }
